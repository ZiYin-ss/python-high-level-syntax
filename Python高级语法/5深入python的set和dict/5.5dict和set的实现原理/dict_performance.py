...
def load_dict_data(total_nums, target_nums):
    ...
    #  这个地方无非不就是在all_data查出target_nums数据放到target_data里面吗
    for x in range(target_nums):
...
#  这个例子是测试查找时间的 注意 dict牛逼

"""
    "abc" ->  ("bc"+随机数) -> hash  
    为什么字典要比列表快 具体的就在于 字典是通过hash直接确定值的 而列表是循环遍历的 所以慢 
"""

#  字典的实现原理就是用了哈希表 可哈希 地址通过key查找 可偏移
...
# 也就是说我们做dict的时候 很少很少关心顺序
# set和dict 原理是一样的吗 都是可哈希的
# 4. 添加数据有可能改变已有数据的顺序
